...
public class GeneralMethods {
    ...
    public void addPointToAClusterAndDeleteFromList(int[] keyClusterAndPoint, List<Cluster> listOfClusters, List<double[]> listOfPoints, boolean showResultInConsole) {
        ...
        if (showResultInConsole == true) {
            System.out.println("Point index  " + pointNumber + " has been successfully added to cluster " + clusterNumber + " and deleted from the original list");
     ...
     */
    public void showFormedClustersSingleLinkage(List<Cluster> listOfClusterWhichNeedToBeNormalized) {
        ...
        List<Cluster> normalizedListOfClusters =  normalizeListOfClusters(listOfClusterWhichNeedToBeNormalized);
        ...
        if (true) {
            ...
            for (int x = 0; x < normalizedListOfClusters.size(); x++) {
                if (normalizedListOfClusters.get(x).getClusterPoints().size() == 0) {
                    System.out.println("__________");
                    System.out.println("##Cluster number " + x + " has no points to be shown");
                } else if (normalizedListOfClusters.get(x).getClusterPoints().size() == 1) {
                    System.out.println("__________");
    ...
    public List<Cluster> normalizeListOfClusters(List<Cluster> listOfClusters) {
        ...
        // create empty clusters for the list
        for (int x = 0; x<listOfClusters.size(); x++){
            tempList.add(new Cluster());
        }
        ...
        // noralize the values in the list of array double
        listOfDouble = normalizeDoubleList(listOfDouble, false);
        // loop through each cluster and then each point to replaced with a normalized value of it based on its ID
        for (int x = 0; x < listOfClusters.size(); x++) {
            for (int y = 0; y < listOfClusters.get(x).getClusterPoints().size(); y++) {
                // through the key value add and replace each point in teh cluster with a normalized one from the normalize one
                double pointId = listOfClusters.get(x).clusterPoints.get(y)[0];
                // search in the list and the point which has the same pointID
                for (int z = 0; z<listOfDouble.size();z++){
                    if (pointId==listOfDouble.get(z)[0]){
                        // add the point to the cluster
                        tempList.get(x).getClusterPoints().add(listOfDouble.get(z));
                    }
                }
            }
        }
